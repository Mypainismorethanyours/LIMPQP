...
def get_config(default_file):
    ...
    # p.add_argument("--quan", default=1, type=int)
    # p.add_argument("--system", default='linux', type=str)
    arg = p.parse_args()
...
def init_logger_win(experiment_name, output_dir, cfg_file=None):
    time_str = time.strftime("%Y%m%d-%H%M%S")
    exp_full_name = time_str if experiment_name is None else experiment_name + '_' + time_str
    log_dir = output_dir / exp_full_name
    log_dir.mkdir(exist_ok=True)
    log_file = log_dir / (exp_full_name + '.log')
    logging.config.fileConfig(cfg_file, defaults={'logfilename': str(log_file).replace('\\', '/')})
    logger = logging.getLogger()
    logger.info('Log file for this run: ' + str(log_file))
    return log_dir
